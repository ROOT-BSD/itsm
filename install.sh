...
if [ "$SKIP_PERMS" -eq 1 ]; then
    ...
    if [ -z "$WEB_USER" ]; then
        ...
        # .git належить тому, хто деплоїть/оновлює код (наприклад, через git pull),
        # а не користувачу веб-сервера — інакше подальші git-операції зламаються.
        # Також повертаємо власника КОРЕНЕВОЇ директорії проєкту (не рекурсивно) —
        # інакше git вважає репозиторій "dubious ownership" (власник каталогу
        # відрізняється від того, хто викликає git), навіть якщо .git виправлено.
        # Група лишається веб-серверною, щоб www-data міг заходити в каталог (traverse).
        if [ -d "${SCRIPT_DIR}/.git" ]; then
            ORIGINAL_USER="${SUDO_USER:-root}"
            if chown -R "$ORIGINAL_USER" "${SCRIPT_DIR}/.git" 2>/dev/null; then
                ok ".git залишено у власності '${ORIGINAL_USER}' (для git pull/push)"
            fi
            chown "${ORIGINAL_USER}:${WEB_GROUP}" "$SCRIPT_DIR" 2>/dev/null
            ok "Кореневу директорію повернуто у власність '${ORIGINAL_USER}' (група ${WEB_GROUP} — для traverse)"
        fi

        # Базові права: директорії 750 (обхід+перелік для власника/групи),
        # файли 640 (читання для власника/групи, без прав на виконання —
        # окрім install.sh, якому виконання поверне git при клонуванні,
        # але тут для повторного ручного запуску відновлюємо явно).
